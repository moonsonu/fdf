...
/*   Updated: 2018/05/09 17:24:16 by ksonu            ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
	...
	while (++i < m->y_max)
	{
		...
		while (++j < m->x_max)
		{
			if (m->pt_new[i][j].z_tmp == 0 && m->pt_new[i][j + 1].z_tmp != 0)
				...
				m->pt_new[i][j].color = 0xbf3eff;
			else if (m->pt_new[i][j].z_tmp != 0 && m->pt_new[i][j].z_tmp < m->pt_new[i][j + 1].z_tmp)
	...
	while (++i < m->x_max)
	{
		...
		while (++j < m->y_max)
		{
			if (m->peter[i][j].z_tmp == 0 && m->peter[i][j + 1].z_tmp != 0)
				...
				m->peter[i][j].color = 0xbf3eff;
			else if (m->peter[i][j].z_tmp != 0 && m->peter[i][j].z_tmp < m->peter[i][j + 1].z_tmp)

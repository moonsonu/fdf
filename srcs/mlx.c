...
/*   Updated: 2018/05/09 17:22:11 by ksonu            ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void		mlx_message(t_env *m)
{
	...
	mlx_string_put(m->mlx_ptr, m->win_ptr, 5, 25, 0xFFFFFF, "X ROTATION : X, V");
	mlx_string_put(m->mlx_ptr, m->win_ptr, 5, 45, 0xFFFFFF, "Y ROTATION : C, D");
	mlx_string_put(m->mlx_ptr, m->win_ptr, 5, 65, 0xFFFFFF, "Z ROTATION : Z, A");
	mlx_string_put(m->mlx_ptr, m->win_ptr, 5, 85, 0xFFFFFF, "MOVE : ARROWS");
	mlx_string_put(m->mlx_ptr, m->win_ptr, 5, 105, 0xFFFFFF, "ZOOM IN/OUT : I, O");
...
void		midpoint(t_env *m)
{
	...
	while (++i < m->y_max)
	{
		...
		while (++j < m->x_max)
		{
				...
				//m->pt_new[i][j].x_tmp = m->pt_new[i][j].x;
				//m->pt_new[i][j].y_tmp = m->pt_new[i][j].y;
		}
	}
}
